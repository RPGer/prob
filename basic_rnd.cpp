...
{
    if(!v.isDef() || uniform0_samp() < tp) { //мутация
        if(!children.empty()) {
            p = children[0]->getValue().getDouble();
        }
        v = (uniform0_samp() < p); //похоже на кроссовер
        v.forceRnd(true);
...
void RndInt_::calcValue(double tp, ExpressionP pPartner)
{
    ...
    if(pPartner && pPartner->name() == name() && pPartner->getValue().isDef()) {
        int perc = rand() % 100; //ниже - кроссовер
		//1 равномерно между родителями
        v.data.n = (v.data.n * perc + pPartner->getValue().data.n * (100 - perc)) / 100;
		//2 случайно из родителей
        //if(perc < 50) v.data.n = pPartner->getValue().data.n;
		//3 равномерно с доп границами
		/*int rad = abs(pPartner->getValue().data.n - v.data.n)/2;
		if (v.data.n>pPartner->getValue().data.n){
			v.data.n = ((v.data.n+rad) * perc + (pPartner->getValue().data.n-rad) * (100 - perc)) / 100;
		} else {
			v.data.n = ((v.data.n-rad) * perc + (pPartner->getValue().data.n+rad) * (100 - perc)) / 100;
		}*/
		//4 нормально с доп границами
		/*int rad = abs(pPartner->getValue().data.n - v.data.n)/2;
		int percent = gaussian_samp(0.5, sqrt(0.5))*100;
		percent = (percent>100?100:percent);
		percent = (percent<0?0:percent);
		if (v.data.n>pPartner->getValue().data.n){
			v.data.n = ((v.data.n+rad) * percent + (pPartner->getValue().data.n-rad) * (100 - percent)) / 100;
		} else {
			v.data.n = ((v.data.n-rad) * percent + (pPartner->getValue().data.n+rad) * (100 - percent)) / 100;
		}*/
		//5 случайно из родителей и небольшое нормальное отклонение
		/*if(perc < 50) v.data.n = pPartner->getValue().data.n;
		perc = rand() % 100;
		int rad = abs(pPartner->getValue().data.n - v.data.n)/2;
		if (v.data.n>pPartner->getValue().data.n){
    ...
    if(!v.isDef()) {
        ...
        randint_resamp(v.data.n, n, tp); //мутация внутри
    }
    logps = log((double)n);
    v.forceRnd(true);
}
